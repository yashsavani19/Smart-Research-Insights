"""
Streamlit dashboard for BERTopic CORE online MVP.
"""
import os
import sys
...
def load_bertopic_model() -> Optional[OnlineBERTopicPipeline]:
    """Load BERTopic model for on-the-fly predictions."""
    try:
        config = load_config()
        ...
        if model_path.exists():
            # Load the model by calling get_topic_summary (which loads it internally)
            pipeline.get_topic_summary()
            return pipeline
        else:
            st.warning("BERTopic model not found. On-the-fly predictions will not be available.")
            return None
    except Exception as e:
...
def main():
    ...
    # Create tabs
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Topics", "Trends", "Search", "Document", "Predict"])
    ...
    with tab5:
        st.header("🔮 Predict Topic")
        
        if bertopic_model:
            st.write("Enter text to predict its topic:")
            
            text_input = st.text_area(
                "Text to analyze:",
                placeholder="Enter research paper title and abstract here...",
                height=200
            )
            
            if st.button("Predict Topic") and text_input.strip():
                with st.spinner("Analyzing text..."):
                    try:
                        topic_id, probability = bertopic_model.transform_text(text_input)
                        
                        if topic_id != -1:
                            # Get topic label from database
                            topics_df = get_topics_data(db_writer)
                            topic_label = topics_df[topics_df['topic_id'] == topic_id]['label'].iloc[0] if not topics_df.empty else f"Topic {topic_id}"
                            
                            st.success(f"**Predicted Topic:** {topic_label}")
                            st.metric("Topic ID", topic_id)
                            st.metric("Confidence", f"{probability:.3f}")
                            
                            # Show topic terms if available
                            try:
                                topic_terms = bertopic_model.model.get_topic(topic_id)
                                if topic_terms:
                                    st.write("**Top Terms:**")
                                    terms_text = ", ".join([term for term, _ in topic_terms[:10]])
                                    st.write(terms_text)
                            except:
                                pass
                        else:
                            st.warning("Text could not be assigned to any topic (outlier).")
                            
                    except Exception as e:
                        st.error(f"Prediction failed: {e}")
        else:
            st.info("BERTopic model not available. Please run the pipeline to train a model.")
